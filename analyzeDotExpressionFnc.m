...
Iblur = imgaussfilt(grayImage,0.5);
[counts] = imhist(Iblur);
counts(1)=0;
x=movmean(counts,3); % make an average of every 3 point

% Find when the idx where the peak is going to decrease
[valX,locsX] = findpeaks(x);
normX = abs(valX)/sum(abs(valX));

idxX =find(normX>0.10);
limX =locsX(idxX(end));

%%
z =diff(x);
% [~,locs] = findpeaks(z,'MinPeakProminence',2000);
[valZ,locsZ] = findpeaks(z);
% figure;
% findpeaks(z)
% Normalize the data
normZ = abs(valZ)/sum(abs(valZ));
idxZ =find(normZ<0.05&locsZ>limX); % find area
lim =locsZ(idxZ(1)); % Threshold when the intensity reach 0 deviation
disp(lim)
%% morphological operations
SE = strel('disk',SIZE_DISK); % Structuring element
...
sdImage = stdfilt(gradmag);
% Run binaryFnc
[overlay,binaryImage] = binaryFnc(sdImage,grayImage);
% Apply median filter on top of the filtered image => increase threshold
J = medfilt2(I_filt);
% Filter size
nhood=[0 1 0; 1 1 1; 0 1 0];
J2 = imopen(J,nhood);
% Add local maximum
fgm1 = imregionalmax(J2,8);
...
D(~binaryImage) = -Inf;
% Add local maxima to the bwdist output, so the watershed transform is
% aware of local peaks
factor = 1.5;
fgm2 =fgm1.*binaryImage; % only look at peaks that have been segmented
D2 = D+(single(fgm2)*-factor);
% Compute the watershed transform.
L = watershed(D2);
% Visualize seperation of objects
% bgm = L == 0;
% figure;imshow(bgm)
%%
...
%% Binary img output
binImg = sum(coloredLabels,3);
binImg(binImg>0)=1;
% figure;imshow(binImg)
% sum(sum(binImg))
result = struct('Original',grayImage,'Image_filt',I_filt,'Circle_Image',coloredLabels_count,'Color_Image',coloredLabels,'Binary_Image',binImg,'Number_blobs',a_l,'Big_Blobs',bigBlob,'Overlay',overlay,'blobMeasurements',blobMeasurements,'lim',lim);
end
...
%% Variables
ARTEFACT_REMOVE=4; % Image regions below 3 pixels are removed
